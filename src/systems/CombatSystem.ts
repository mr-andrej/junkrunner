...
export class CombatSystem {
  executeJob(state: GameState): GameState {
    if (!state.activeJob) {
      return state.addLog('No active job. Use "take <#>" to accept one.', 'error');
    }
    ...
    if (success) {
      ...
      const repKey = (isBlack ? 'repBlackMarket' : 'repFixer') as keyof GameState;
